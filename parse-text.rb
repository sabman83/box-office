...
while lines_num < lines.length && theater_num < THEATERS.length do
 ...
 begin #loop through each line of theater's data
   ...
   db.execute('INSERT OR IGNORE INTO weekly_performance(movie_id, theater_id, weekend_date, weekend_gross, last_weekend_gross, weekly_gross, last_weekly_gross) VALUES (?,?,?,?,?,?,?)',
                                              movie_id, theater_id, week_start_date.to_s, data_line[:this_weekend_gross], data_line[:last_weekend_gross], data_line[:this_week_gross], data_line[:last_week_gross] )
