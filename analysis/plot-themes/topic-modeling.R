...
doc.vec<-VectorSource(as.character(movie_info_corpus$plot))
doc.corpus<-Corpus(doc.vec)
doc.corpus <- tm_map(doc.corpus, tolower)
doc.corpus <- tm_map(doc.corpus, removePunctuation)
doc.corpus <- tm_map(doc.corpus, removeNumbers)
doc.corpus <- tm_map(doc.corpus, removeWords, stopwords("english"))
doc.corpus <- tm_map(doc.corpus, removeWords, stop_words$word)
...
summary(term_tfidf)
DTM<-DTM[,term_tfidf >= 0.65] # use median of term_tfidf
rowTotals <- apply(DTM , 1, sum)
doc.lda<- LDA(DTM[rowTotals>0,], k = 40, control = list(seed = 1234))
...
iter <- 1000
keep <- 50
system.time(fitted_many <- lapply(seqk, function(k) topicmodels::LDA(DTM[rowTotals>0,], k = k,
...
movie.corpus <- tm::Corpus(tm::DataframeSource(movie_info_corpus), readerControl = list(reader = tm::readTabular(mapping = m)))
movie.dtm <- tm::DocumentTermMatrix(movie.corpus, control = list(stemming = TRUE, stopwords = TRUE,
minWordLength = 2, removeNumbers = TRUE, removePunctuation = TRUE))
str(movie.dtm)
term_tfidf <- tapply(movie.dtm$v/slam::row_sums(movie.dtm)[movie.dtm$i], movie.dtm$j, mean) *
log2(tm::nDocs(movie.dtm)/slam::col_sums(movie.dtm > 0))
summary(term_tfidf)
movie.reduced.dtm <- movie.dtm[,term_tfidf >= 0.58]
summary(slam::col_sums(movie.reduced.dtm))
seqk <- seq(2, 50, 1)
burnin <- 1000
iter <- 1000
keep <- 50
system.time(fitted_many <- lapply(seqk, function(k) topicmodels::LDA(movie.reduced.dtm[rowTotals>0,], k = k,
method = "Gibbs",control = list(burnin = burnin,
iter = iter, keep = keep) )))
...
ldaplot
movie.model <- topicmodels::LDA(movie.reduced.dtm[rowTotals>0,], 22, method = "Gibbs", control = list(iter=2000, seed = 0622))
movie.topics <- topicmodels::topics(movie.model, 1)
movie.terms <- as.data.frame(topicmodels::terms(movie.model, 30), stringsAsFactors = FALSE)
movietopics.df <- as.data.frame(movie.topics)
movietopics.df <- dplyr::transmute(movietopics.df, imdb_id = rownames(movietopics.df), Topic = movie.topics)
movie_info_corpus<-dplyr::inner_join(movie_info_corpus, movietopics.df, by = "imdb_id")
...
movie.json <- topicmodels_json_ldavis(movie.model, movie.corpus, movie.reduced.dtm[rowTotals>0,])
